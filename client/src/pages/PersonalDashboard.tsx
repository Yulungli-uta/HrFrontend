...
export default function PersonalDashboard() {
  ...
  const [tab, setTab] = useState<"resumen" | "permisos" | "vacaciones" | "justificaciones" | "marcaciones">(
    "resumen"
  );
  const [qPerm, setQPerm] = useState("");
  const [qVac, setQVac] = useState("");
  const [qJust, setQJust] = useState("");
  ...
  // ✅ Ajuste: capturamos isLoading y error también para last punch
  const {
    data: lastPunchRes,
    isLoading: loadingLastPunch,
    error: errorLastPunch,
  } = useQuery({
  ...
  const {
    data: todayPunchesRes,
    isLoading: loadingPunches,
    error: errorPunches,
  ...
  const perms: Permission[] = useMemo(
    () =>
      safeArray(permsRes).map((p: any) => ({
        ...
        approvedAt: p.approvedAt ?? p.ApprovedAt,
  ...
  const justifications: Justification[] = useMemo(
    () =>
      safeArray(justRes).map((j: any) => ({
  ...
  const todayPunches: Punch[] = useMemo(
    () =>
      safeArray(todayPunchesRes).map((p: any) => ({
  ...
  const lastPunch: Punch | null = useMemo(() => {
    const d = (lastPunchRes as any)?.data ?? lastPunchRes;
    if (!d) return null;
    return {
  ...
  const filteredPerms = perms
    ...
    .sort(
      (a, b) =>
        new Date(b.createdAt ?? b.requestDate ?? b.startDate ?? 0).getTime() -
  ...
  const filteredJusts = justifications
    ...
    .sort(
      (a, b) =>
        new Date(b.justificationDate ?? b.startDate ?? 0).getTime() -
  ...
  /* -------- errores -------- */
  const anyError = errorPerms || errorVacs || errorJust || errorPunches || errorLastPunch;
  const errorMsg = anyError ? handleApiError(anyError, "No se pudieron cargar todos los datos.") : null;

  // ✅ Ajuste clave: Loader global SOLO en la primera carga (isLoading)
  // Esto evita que se “bloquee” cada 30s por el refetchInterval.
  const isBootLoading =
    !!employeeId && (loadingPerms || loadingVacs || loadingJust || loadingPunches || loadingLastPunch);

  // ✅ Espera a tener employeeId (si tu AuthContext tarda)
  if (!employeeId) {
    return (
  ...
  // ✅ Pantalla de carga hasta que terminen TODAS las APIs
  if (isBootLoading) {
    return (
      <div className="container mx-auto p-6">
        <Card>
          <CardContent className="p-10 flex flex-col items-center gap-3">
            <RefreshCw className="h-5 w-5 animate-spin" />
            <div className="text-sm text-muted-foreground">Cargando información del panel…</div>
          </CardContent>
        </Card>
      </div>
    );
      ...
      <Card className="border-dashed">
        ...
        {lastPunch && (
          <CardContent className="pt-0 text-sm text-muted-foreground flex flex-wrap items-center gap-3">
            {lastPunch.deviceId && (
              <>
                Dispositivo: <Badge variant="secondary">{lastPunch.deviceId}</Badge>
              </>
            )}
          ...
          <Card>
            ...
            <CardContent className="p-0">
              {loadingPunches ? (
                ...
                <PunchTable punches={todayPunches} />
              )}
            </CardContent>
          </Card>
        </TabsContent>
                          ...
                          return (
                            <TableRow key={`perm-${p.id ?? i}`}>
                              ...
                              <TableCell className="max-w-[360px]" title={p.justification ?? ""}>
                                {(p.justification ?? "—").length > 80
                                  ? (p.justification ?? "—").slice(0, 80) + "…"
                                  : p.justification ?? "—"}
                              </TableCell>
                            </TableRow>
                          );
                        ...
                        <Card key={`just-m-${j.punchJustId ?? i}`} className="p-3">
                          ...
                          <div className="mt-2 text-xs grid grid-cols-2 gap-2">
                            <div>
                              ...
                              {fmtDate(j.justificationDate ?? j.startDate ?? undefined)}
                            </div>
                            <div>
                              <span className="text-muted-foreground">Hasta: </span>
                              {fmtDate(j.endDate ?? undefined)}
                            </div>
                            <div>
