...
export function DepartmentParentSelect({
  ...
  const { data, isFetching } = useQuery({
    queryKey: ['vw-departments-parent', departmentScopeId ?? 'all'],
    queryFn: () =>
      departmentScopeId
  ...
  const filtered = useMemo(() => {
    ...
    return allDepts.filter((d) => {
      if (excludeDepartmentId && d.departmentID === excludeDepartmentId) return false;
      if (!term) return true;
      return (
  ...
  }, [allDepts, search, excludeDepartmentId]);
