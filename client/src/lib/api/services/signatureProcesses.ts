...
import type { AuditEvent, CreateSigningRequest, FirmaEcCertificateType, SigningDocument, SigningProcessListItem, SigningProgress, SigningSession } from "@/types/electronic-signature";
export const SignatureProcessesAPI={
 ...
 startSigning:(id:number,position?:{page:number;llx:number;lly:number;width?:number;height?:number},certificateType?:FirmaEcCertificateType)=>{const body={...position,certificateType};return apiFetch<SigningSession>(`/api/v1/signature/processes/${id}/start-signing`,{method:"POST",body:JSON.stringify(body)});},
